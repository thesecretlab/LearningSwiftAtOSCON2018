...
getRandomBool { // this requires a block or function to be passed as its parameter
    return Int(arc4random_uniform(10)) // get a random integer 0 - 9
}
...
func shuffle(_ array: [String]) -> [String] {
    ...
    var oldArray = array        // a copy of the original array
    var newArray: [String] = [] // an empty array to put the rearranged values into
    var index: Int              // a random selector
    
    // as long as there are values in the old array that aren't in the new one
    while oldArray.count > 0 {
        
        index = Int(arc4random_uniform(UInt32(oldArray.count))) // get a random index
        newArray.append(oldArray[index])                        // put the value from the old array at that index next in the new array
        oldArray.remove(at: index)                              // remove that value from the old array
    }
...
enum Day: String { // you can "extend" a type to give each value a "rawValue" of that Type
    ...
    func increment(days number: Int) -> Day {
        ...
        let days = self.cases
        var index = days.index(of: self) ?? 0
...
/*:
 ...
 There are some things that Playgrounds is *not so good at*. I/O and Debugging begins to uncover some of the comparative limitations of the environment.

 To get data into Playgrounds, we require a FileManager. This handling is the same regardless of what you are bringing in, they will just require different encoding/decoding parameters to convert from the intermediate *Data?* type.
 */
let fileManager = FileManager.default
//: We can get an existing local file.
var dataIn: String? = nil

// get the filepath of the Resources directory, plus filename
if let filepath = Bundle.main.path(forResource: "file", ofType: "txt") {
    
    // read file with your given contents from it
    dataIn = try? String(contentsOfFile: filepath)
} else {
    
    print("ERROR: File was unable to be read!")
}
...
users?.first?.email
//: Also, using the FileManager, we are able to delete files. This is handy to cleanup things you have created.
func deleteFile(_ urlString: String) {
    do {
        ...
        print("ERROR: Unable to remove file! \(error)")
    }
}
/*:
 Unlike Xcode, however, Playgrounds does not award permissions to write outside of its own directories. So we cannot *create* a file this way. Instead, Playgrounds offers optional use of a Documents/Shared Playground Data folder that will be accessible to any Playground you make. For this to be possible, the Directory must already exist.
 
 You can also *technically* write to your Playgrounds' Resources folder, but you file won't appear in the folder you can see within the Playgrounds' Package Contents. Instead it will write to the T (temp) folder that this represents, deep in the bowels of your hidden folder heirarchy. I don't imagine this is recommended.
 
 Keep in mind that if you write to the same place again your file will not be appended, simply replaced with a file with the new contents. There are libraries that do better, but the only bare-bones solution is to copy the contents then append the new contents and write back both.
 */
let documentsFilepath = playgroundSharedDataDirectory.appendingPathComponent("file.txt")
let stringOut = "This tests writing TO a file."
let dataOut = stringOut.data(using: .utf8)


// write to Documents/Shared Playgrounds Data
do {
    try dataOut?.write(to: documentsFilepath)
} catch {
...
// write to /var/folders/ ... /resources/
if let path = Bundle.main.path(forResource: "file", ofType: "txt") {
    do {
        path
        let sandboxFilepath = URL(fileURLWithPath: path)
        try dataOut?.write(to:sandboxFilepath)
 ...
 * print statements
 ...
 With the presence of live values in the right sidebar, we effectively have a tracer to show what each variable holds at every point and how many times each individual line is run. We also have the classic print() statements, which will output to the log footer as expected.
...
func decreasingRandomNumbers(numbering count: Int, upTO limit: Int) -> [Int] {
    ...
    for _ in 1...count {
        ...
        random = Int(arc4random_uniform(UInt32(limit + 1))) // generate random number between 0 and limit
